...
def main(argv: list[str] | None = None) -> int:
    ...
    observed: dict[str, Any] = {"approvals": 0, "terminalStatus": None}
    bridge = CodexBridge(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="retro-web-ui-app-server-smoke-") as directory:
            ...
            bridge.start_turn(
                thread_id,
                [{"type": "text", "text": "Protocol smoke only. Do not edit files or run commands. Reply with: READY"}],
                cwd=str(root),
            ...
            result = {
                "status": "ok" if observed["terminalStatus"] in {"completed", None} else "error",
                "model": args.model,
