...
workspace ships in the v2.1.4 native archives.
...
[v2.1.4 release](https://github.com/ririri-rgb/retro-web-ui/releases/tag/v2.1.4).
...
the v2.1.4 Linux archive is built on Ubuntu 22.04 and gated at GLIBC 2.35 plus a
...
Version `2.1.4` is the current desktop + CLI + Skill release. It retains the standalone Skill layout, installable deterministic CLI, and the `v1.0.0` legacy helper entry points.
...
- Native packages are not Developer ID-notarized or Authenticode-signed in v2.1.4, and there is no auto-updater. Verify checksums and use the operating system's explicit local-app approval flow.
