...
the v2.0.1 Linux archive is built on Ubuntu 22.04 and gated at GLIBC 2.35 plus a
...
Version `2.0.1` is the current desktop + CLI + Skill release. It retains the standalone Skill layout, installable deterministic CLI, and the `v1.0.0` legacy helper entry points.
...
- Native packages are not Developer ID-notarized or Authenticode-signed in v2.0.1, and there is no auto-updater. Verify checksums and use the operating system's explicit local-app approval flow.
