...
このworkspaceはv2.1.1 native archiveに含まれます。

![Retro Web UI desktop GUI](screenshots/gui/desktop-xp.png)

[v2.1.1 release](https://github.com/ririri-rgb/retro-web-ui/releases/tag/v2.1.1)から
macOS arm64 / Windows x86_64 / Linux x86_64用native archiveを取得できます。PythonとQtは同梱しますが、Codexは同梱しません。
macOS版は検証済みad-hoc署名ですがnotarizeされておらず、Windows版はunsignedです。v2.1.1 Linux版はUbuntu 22.04でbuildし、GLIBC 2.35以下をgateとします。通常のdesktop display stackも必要です。起動前にSHA-256を確認してください。
...
リポジトリ限定で使う場合は `.agents/skills/retro-web-ui/` へコピーします。Skill本体とCLI runtimeはPython 3.9以上だけで動作し、第三者runtime packageは不要です。`v2.1.1`は現在のdesktop + CLI + Skill releaseで、standalone Skill構成と`v1.0.0`のlegacy helper entry pointを維持しています。
...
- v2.1.1 native archiveはDeveloper ID notarization / Authenticode signingを行っておらず、自動更新もありません。checksumを確認し、OSの明示的なlocal-app許可手順を使ってください。
